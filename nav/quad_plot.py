...
class Planner:
    def __init__(self, start_state, end_state, cfg, density_fn):
        ...
        self.mass               = cfg['mass']
        self.J                  = cfg['I']      ## I is initialled as inertial tensor 
        ...
        self.dt = self.T_final / self.steps    # 2/20

        self.start_state = start_state
        self.end_state   = end_state

        slider = torch.linspace(0, 1, self.steps)[1:-1, None]   # [18,1]
        ...
        self.initial_accel = torch.tensor([cfg['g'], cfg['g']]).requires_grad_(True)     ## why just have 2 accel
    ...
    def full_to_reduced_state(self, state):   ## mark.. how can we get the reduced state
        pos = state[:3]
    ...
    def a_star_init(self):
        ...
        # print(self.states.shape)    ## shape (18,4)
        ...
        if self.CHURCH:
            ...
            # side, side, side, 3
            coods = torch.stack( torch.meshgrid( linspace, linspace, linspace ), dim=-1)    # 100 100 100 3

        kernel_size = 5 # 100/5 = 20. scene size of 2 gives a box size of 2/20 = 0.1 = drone size
        output = self.nerf(coods)    # 100 100 100
        ...
        # 20, 20, 20
        occupied = maxpool(output[None,None,...])[0,0,...] > 0.3   # shape 20 20 20

        grid_size = side//kernel_size  ## 100 / 20 = 5

        #convert to index cooredinates
        start_grid_float = grid_size*(self.start_state[:3] + 1)/2    #  convert to grid coordinates 
        ...
        path = astar(occupied, start, end)    # get the original path by using A star

        # convert from index cooredinates
        squares =  2* (torch.tensor( path, dtype=torch.float)/grid_size) -1   # shape (21,3)

        #adding way
        states = torch.cat( [squares, torch.zeros( (squares.shape[0], 1) ) ], dim=-1)   ## add one more dimension (last col)
        ...
        states = (prev_smooth + next_smooth + states)/3    ## shape (21,4)
    ...
    def calc_everything(self):
        ...
        start_pos   = self.start_state[None, 0:3]                   # position  
        start_v     = self.start_state[None, 3:6]                   # linear velocity
        start_R     = self.start_state[6:15].reshape((1, 3, 3))     # rotation matrix
        start_omega = self.start_state[None, 15:]                   # angular matrix
        ...
        next_R = next_rotation(start_R, start_omega, self.dt)  ## get the next rotation matrix

        # start, next, decision_states, last, end

        start_accel = start_R @ torch.tensor([0,0,1.0]) * self.initial_accel[0] + self.g     # why this is the right way to calculate accel
        next_accel = next_R @ torch.tensor([0,0,1.0]) * self.initial_accel[1] + self.g

        next_vel = start_v + start_accel * self.dt      ## get the velocity
        after_next_vel = next_vel + next_accel * self.dt

        next_pos = start_pos + start_v * self.dt        ## get the position
        ...
        current_accel = (next_vel - prev_vel)/self.dt - self.g    # after minus g, we get pure acceleration for robot before force combination
        ...
        current_omega = rot_matrix_to_vec( rot_matrix[1:, ...] @ rot_matrix[:-1, ...].swapdims(-1,-2) ) / self.dt      # swapdims actually get the transpose of a given matrix
        ...
        # S, 3    3,3      S, 3, 1
        torques = (self.J @ angular_accel[...,None])[...,0]               ## J is inertial tensor  τ = I ⋅ α
    ...
    def get_state_cost(self):
        ...
        fz = actions[:, 0].to(device)      ### just the magnitude of linear_accel x mass
        torques = torch.norm(actions[:, 1:], dim=-1).to(device)    # overall torques

        # S, B, 3  =  S, _, 3 +      _, B, 3   X    S, _,  3
        B_body, B_omega = torch.broadcast_tensors(self.robot_body, omega[:,None,:])     # 24 500 3      24 500 3     traj_point(state_num) body_cloud_point(body_size) omega(s,3)
        point_vels = vel[:,None,:] + torch.cross(B_body, B_omega, dim=-1)

        # S, B
        distance = torch.sum( vel**2 + 1e-5, dim = -1)**0.5             ## get the distance in SE3
        # S, B
        density = self.nerf( self.body_to_world(self.robot_body) )**2       #  why we have square here ? to emphasize density? 
