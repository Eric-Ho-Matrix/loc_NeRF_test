...
def get_pose(phi, theta, psi, x, y, z, initial_pose, center_about_true_pose):
    if center_about_true_pose:
        # print("recentering")
        # print(obs_img_pose)
        pose = trans_t(x, y, z) @ rot_phi(phi/180.*np.pi) @ rot_theta(theta/180.*np.pi) @ rot_psi(psi/180.*np.pi)  @ initial_pose
    else:
